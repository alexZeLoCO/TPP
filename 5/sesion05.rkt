...
(define (filtrar-for f . l)
  (lambda (l)
    (map (curry filter f) l)
    )
  )

(displayln "filter-for:")
(filtrar-for atom? '(1 (2) 3) '(9 (2 3)) '(0 1 6))      ; => ((1 3) (9) (0 1 6))
(filtrar-for number? '(a (b) 3) '(d (2 e)) '(a 1 (b)))  ; => ((3) () (1))
...
(define info
  (lambda (key p)
    (cdar (filter (curry member key) p))
    )
  )
...
(define buscarUna
  (lambda (keys p)
    (map (lambda (key) (info key p)) keys)
    )
  )
       
(displayln "buscarUna")
(buscarUna '(nombre apellidos) (cadr Datos))

(define buscar+
  (lambda (keys datos)
    (map (curry buscarUna keys) datos)
    )
  )
...
(define (change-allC l u v)
  (map (lambda (x) (choose x v u)) l)
  )

(display "change-all: ")
(change-allC '((a) b 2 (a 3) (a) a) '(a) 0)  ; => (0 b 2 (a 3) 0 a)
