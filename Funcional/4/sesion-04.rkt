...
; previamente que su número de argumentos es el esperado y que éstos
...
(define union               
  (lambda resto                    
	...
	[else (cons (car A)
      ...
      (cond ([not (= (length resto) 2)]
             (error "union(A, B): requiere dos argumentos"))
            ([not (list? (car resto))]
             (error "union(A, B): el primer argumento debe ser una lista"))
            ([not (list? (cadr resto))]
             (error "union(A, B): el segundo argumento debe ser una lista"))
            (else
             (f (car resto) (cadr resto)))))))


(display "union: ")
;(union '(c a x) 3)       ;=> error (OK)
;(union '(c a x))         ;=> error (OK)
...
(define (subset?Rec A B)
  (or (null? A)(_and (member (car A) B) (subset? (cdr A) B)))
  )

(define (subset? A B)
  (equal? (filter (lambda (x) (member x B)) A) A)
  )
...
(define (subset2? A B)
  (apply _and (map (lambda (x) (member x B)) A))
  )
...
(define (suc-aritmetica a r n)
  (map (lambda (x) (+ a (* x r))) (enteros 0 n))
  )
...
(define (multiplos a b n)
  (filter (lambda (x) (zero? (remainder n x))) (enteros a b)) ;WIP
  )
