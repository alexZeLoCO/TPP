...
;;-----------------------------------------------------------------------------------------
;
; Dado que  l1, l2, ... es una secuencia de un número indeterminado de listas y a todas
; éstas hay que concatenarlas la lista l, que es un argumento fijo, la recurrencia se
; establecerá sobre la lista de argumentos opcionales: resto = (l1 l2 ...)
;
; 1. Base       : resto es la lista vacía addLast(l) = ()
; 2. Recurrencia: resto no es la lista vacía resto = cons(car(resto), cdr(resto))
;                 si resto = (l1 l2 l3 ...), entonces car(resto)=l1 y cdr(resto)=(l2 l3 ...)
;      Hipótesis: se conoce addLast(l, l2, l3, ...); es decir, apply(addLast, cons(car(l), cdr(resto))) = H
;          Tesis: addLast(l, l1, l2, ...) = cons(append(car(resto)), l, H)

(define (addLast l . resto)
  (cond [(null? resto) resto]
        [else (cons (append (car resto) l)
                    (apply addLast l (cdr resto)))]))
...
(define (addLast+ ll . resto)
  (map (lambda (x) (append x (apply append ll))) resto)
)
...
;;------------------------------------------------------------------------------------
;; Definir la función cartas-mano(nombre, manos) que dado el nombre de un jugador de
;; las manos de una partida del UNO retorna las cartas de éste:
;; la lista (nombre-del-jugador (mano-del-jugador)).
;;------------------------------------------------------------------------------------

(define (cartas-mano nom manos)
  (car (filter (curry member nom) manos))
  )
...
;(define (comodin? cartas-mano)
 ;(not (empty? (filter (lambda (x) (map (lambda (elem) (eq? (length elem) 1)) x)) (cdr cartas-mano))))
;  (filter (lambda (x) (map (lambda (elem) (eq? (length elem) 1)) x)) (cdr cartas-mano))
;  )

(define (comodin? cartas)   ; (maria ((7 verde) (8 amarillo) (0 rojo) (comodin-roba4) (3 verde) (8 verde) (3 azul)))
  (not (empty? (filter (lambda (carta) (equal? (length carta) 1)) (cadr cartas))))
  )
...
(define (valor-compatible valor cartas)
  (filter (lambda (carta) (or (equal? (length carta) 1) (equal? (car carta) valor))) (cadr cartas))
  )
...
(define (color-compatible color cartas)
    (filter (lambda (carta) (or (equal? (length carta) 1) (equal? (cadr carta) color))) (cadr cartas))
)

(display "color-compatible (verde, cartas de maría): ")
(color-compatible 'verde (cartas-mano 'maria Manos)) ;=> ((7 verde) (3 verde) (8 verde) (comodin-roba4))
(display "color-compatible (rojo, cartas de pedro): ")
...
(define (compatibles carta_objetivo cartas)
  (filter (lambda (carta) (or (equal? (length carta) 1) ; Caso 1: La carta es un comodín
                              (equal? (cadr carta) (cadr carta_objetivo)) ; Caso 2: Los colores de las cartas coinciden
                              (equal? (car carta) (car carta_objetivo)) ; Caso 3: Los números de las cartas coninciden
                              ))
            (cadr cartas))
)

(display "compatibles (9 rojo): ")
(compatibles '(9 rojo)
             (cartas-mano 'ana Manos)) ;=> ((4 rojo) (9 rojo) (9 verde) (comodin-color) (9 verde))
(display "compatibles (comodin-color verde): ")
...
(define (jugadores-con-comodin manos)
  (filter (lambda (mano) (comodin? (cartas-mano mano manos))) (map car manos))
  )
...
(define (jugadores-cartas-compatibles carta_objetivo manos)
  (map (lambda (mano) (cons (car mano) (compatibles carta_objetivo mano))) manos)
  )
...
(define (suma l-nombres l-vectores)
  (cond [(empty? l-vectores) 0]
        [(member (car l-vectores) l-nombres) (+ (apply + (cadr l-vectores)) (suma l-nombres (cddr l-vectores)))]
        [else (suma l-nombres (cddr l-vectores))]
        )
  )
...
(define (producto x l-nombres l-vectores)
  (cond [(empty? l-vectores) '()]
        [(member (car l-vectores) l-nombres) (cons (cons (car l-vectores) (list (por-escalar x (cadr l-vectores)))) (producto x l-nombres (cddr l-vectores)))]
        [else (cons (cons (car l-vectores) (list (cadr l-vectores))) (producto x l-nombres (cddr l-vectores)))]
        )
  )
...
(define (producto x l-nombres l-vectores)
  (cond [(empty? l-vectores) '()]
        [(member (car l-vectores) l-nombres) (cons (cons ((string-append (symbol->string x) (string '*) (string car l-vectores)))
                                                         (list (por-escalar x (cadr l-vectores)))) (producto x l-nombres (cddr l-vectores)))]
        [else (cons (cons (car l-vectores) (list (cadr l-vectores))) (producto x l-nombres (cddr l-vectores)))]
        )
  )
